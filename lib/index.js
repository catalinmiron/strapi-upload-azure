    ...
    return {
      upload: (file) => {
        return new Promise((resolve, reject) => {
          function process(file){
            var blobName = file.path ? file.path + `/${file.hash}${file.ext}` : config.defaultPath ? `${config.defaultPath}/${file.hash}${file.ext}` : `${file.hash}${file.ext}`;
            var blobURL = BlobURL.fromContainerURL(file.isPrivate ? privateContainerUrl : containerURL, blobName);
      ...
      delete: (file) => {
        return new Promise((resolve, reject) => {
          ...
          .then(function(){
            if([jimp.MIME_PNG, jimp.MIME_JPEG, jimp.MIME_BMP].indexOf(file.mime) > -1){
              blobName = blobName.replace(file.hash + file.ext, 'thumb-' + file.hash + file.ext);
              blobURL = BlobURL.fromContainerURL(file.isPrivate ? privateContainerUrl : containerURL, blobName);
      ...
      download: (file) => {
        return new Promise((resolve, reject) => {
          var blobName = file.path ? file.path + `/${file.hash}${file.ext}` : config.defaultPath ? `/${file.hash}${file.ext}` : `${file.hash}${file.ext}`;
          var blobURL = BlobURL.fromContainerURL(privateContainerUrl, blobName);
          var blockBlobURL = BlockBlobURL.fromBlobURL(blobURL);
